...
export function renderTimeline() {
  ...
  const orderElem = document.getElementById("timelineOrder");
  const personElem = document.getElementById("timelinePersonFilter");
  const locationElem = document.getElementById("timelineLocationFilter");
  const typeElem = document.getElementById("timelineTypeFilter");

  const order = orderElem ? orderElem.value : "asc";
  const personFilter = personElem ? personElem.value : "";
  const locationFilter = locationElem ? locationElem.value : "";
  const typeFilter = typeElem ? typeElem.value : "";
  ...
  for (let e = 0; e < events.length; e++) {
    ...
    for (let el = 0; el < item.locationIds.length; el++) {
      ...
      eventLocationNames.push(evtLoc ? (evtLoc.name || evtLoc.id || item.locationIds[el]) : item.locationIds[el]);
    }
